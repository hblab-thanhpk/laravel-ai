...
@section('content')
    <div class="page-header">
        <div>
            ...
            <p class="muted">Quản trị các quyền hệ thống và gán vào role.</p>
        </div>

        <a class="button" href="{{ route('admin.permissions.create') }}">Tạo permission</a>
    </div>

    <div class="card">
        <form action="{{ route('admin.permissions.index') }}" class="toolbar" method="GET">
            ...
            <div class="form-actions" style="margin-top: 0;">
                ...
                @if ($filters['search'] !== '' || $filters['sort_by'] !== 'created_at' || $filters['sort_dir'] !== 'desc' || (int) $filters['per_page'] !== 10)
                    <a class="button-link" href="{{ route('admin.permissions.index') }}">Xóa bộ lọc</a>
                @endif
            </div>
        </form>
    </div>
    ...
    <div class="card">
        @if ($permissions->count() === 0)
            <div class="empty-state">Chưa có permission nào.</div>
        @else
                    ...
                    <tbody>
                        @foreach ($permissions as $permission)
                            <tr>
                                ...
                                <td><span class="inline-code">{{ $permission->name }}</span></td>
                                <td>{{ $permission->roles_count }}</td>
                                <td>
                                    <span class="role-badge {{ $permission->is_system ? 'admin' : 'user' }}">
                                        {{ $permission->is_system ? 'System' : 'Custom' }}
                                    </span>
                                </td>
                                <td>
                                    <div class="table-actions">
                                        <a class="button-secondary" href="{{ route('admin.permissions.show', $permission) }}">Xem</a>
                                        <a class="button-secondary" href="{{ route('admin.permissions.edit', $permission) }}">Sửa</a>
            ...
            @if ($permissions->hasPages())
                <div class="pagination">
                    <span class="muted">Hiển thị {{ $permissions->firstItem() }} - {{ $permissions->lastItem() }} trên tổng {{ $permissions->total() }} permissions</span>

                    <div class="pagination-links">
                        @if ($permissions->onFirstPage())
                            <span class="pagination-link is-disabled">Trước</span>
                        @else
                            <a class="pagination-link" href="{{ $permissions->previousPageUrl() }}">Trước</a>
                        @endif

                        <span class="pagination-link">{{ $permissions->currentPage() }}/{{ $permissions->lastPage() }}</span>

                        @if ($permissions->hasMorePages())
                            <a class="pagination-link" href="{{ $permissions->nextPageUrl() }}">Sau</a>
                        @else
