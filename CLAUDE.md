...
Chi tiết từng chủ đề nằm trong `.claude/` — **đọc file phù hợp trước khi generate code**:

| File | Nội dung |
|------|----------|
| `.claude/copilot-context.md` | Domain knowledge, business flows, system rules |
| `.claude/architecture.md` | Layer rules, coding standards, naming, transaction patterns |
| `.claude/admin-rules.md` | Admin Panel (Blade SSR) — view, form, pagination, flash |
| `.claude/api-rules.md` | REST API — response format, Sanctum, Resource, throttling |
| `.claude/database.md` | PostgreSQL, UUID, migrations, Eloquent rules |
<!-- | `.claude/devops.md` | Docker Compose, GitHub Actions CI/CD | -->
